...
Prisma-AI 是一个开源免费的AI求职助手，旨在帮助用户优化项目、定制简历、匹配工作并准备面试。它基于先进的AI架构，包括Planer-Executor、CRAG和Human-in-the-loop，能够将想法转化为可执行计划。项目还集成了面试经验驱动的学习系统，并提供IT求职数据网站（https://pinkprisma.com）。最新版本包括简历PDF编辑与导出、DeepWiki集成以及AI助手Prisma等功能。

## 功能

- **AI核心**：基于LangChain、LangGraph和fastmcp等技术，提供精准上下文支持，支持多轮反馈优化输出质量。
- **简历优化与落地**：深度分析项目，自动打磨文案，挖掘可落地亮点，并通过AI Agent实现项目亮点。支持简历PDF编辑与导出。
...
- **DeepWiki集成**：支持将DeepWiki作为项目知识库，用于快速创建项目知识库。
- **AI助手Prisma**：通过用户记忆进行简历优化、人岗匹配，提供更好的知识库检索和针对性学习。
