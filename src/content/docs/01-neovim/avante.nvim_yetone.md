...
> Description from GitHub: Use your Neovim like using Cursor AI IDE!
...
| **项目特定指令文件** | 通过在项目根目录添加 markdown 文件（默认为 avante.md）来自定义 AI 行为。  |
| **Avante Zen Mode**  | 特殊模式，模仿在 Neovim 中使用 Cursor AI IDE 的体验。                     |
| **快速应用**         | 使用专用模型实现高精度的即时代码编辑。                                    |
| **ACP 支持**         | 与代理客户端协议集成，支持 Claude Code、Codex 和 Gemini CLI 等代理。      |
| **自定义工具**       | 定义 AI 在代码生成和分析期间可以使用的自定义工具。                        |
