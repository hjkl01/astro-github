...
5. **使用步骤**：
   - 浏览器访问部署地址，注册管理员账号。
   - 登录后，创建存储策略，选择本地存储、阿里云 OSS、腾讯云 COS、OneDrive、Google Drive 等。
   - 上传文件，通过 Web 界面管理（上传、下载、删除、重命名、移动、复制）。
   - 使用搜索功能查找文件，支持全文搜索和标签管理。
   - 配置用户角色和权限，确保数据安全。
   - 自定义插件：在 `zfile-plugin` 目录开发并加载，支持水印、转码、OCR 等扩展。
   - 查看操作日志和性能监控。
