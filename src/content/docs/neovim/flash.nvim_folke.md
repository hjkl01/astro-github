...
> Description from GitHub: Navigate your code with search labels, enhanced character motions and Treesitter integration
...
- **VS Code**: some functionality is changed/disabled when running **flash** in **VS Code**:
  - `prompt` is disabled. You can use `require("flash").prompt()` to get
    the prompt text and integrate it into the statusline.
